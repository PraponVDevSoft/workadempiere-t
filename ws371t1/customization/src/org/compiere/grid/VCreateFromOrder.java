...
//		List<MOrderLine> newOrderLineList = new ArrayList<MOrderLine>();
	...
	protected Vector<String> getBOMColumnNames() {
		...
		columnNames.add("จำนวนต่อตัว");								// columnNames.add("Qty Required");
		columnNames.add("หน่วย");
// aon 2/4/57		columnNames.add(Msg.translate(Env.getCtx(), "สั่งซื้อล่าสุด.")); // เดิม column 2-aon 12/3/56
		return columnNames;
	}
...
// aon 2/4/57		miniTable.setColumnClass(5, String.class, true);     // 2-Description
		miniTable.autoSize();
	}
	...
	protected Vector<Vector<Object>> getBOMData(int M_Product_ID) {
		...
		for (int i = 0; i < bomLines.length; i++) {
			...
			System.out.println(" for:"+i+product.getName());
...
// aon --สลับ column 12/3/56 -----------		
			...
			line.add(attSetIns);
			System.out.println("Att Set Instance : "+M_AttSetIns_Desc+" attSetIns "+attSetIns+" id : "+M_AttributeSetInstance_ID);	// aon 22/4/57
...
/* aon 2/4/57 
			...
			line.add(descpo);			
*/
			System.out.println("Bom ID:"+PP_Product_BOM_ID+"  Att ID:"+M_AttributeSetInstance_ID);
/*	-----------------------------------------		
			String sql2 = "select 'yes' as found from pp_product_bomline bl " +
                          "where bl.PP_Product_BOM_ID = " + PP_Product_BOM_ID +
						  "  and bl.m_attributesetinstance_id = " + M_AttributeSetInstance_ID +
                          "  and bl.m_attributesetinstance_id not in ( " +
	                      "select l.m_attributesetinstance_id from c_order o,c_orderline l "+
		                  "  where o.c_order_id = l.c_order_id and o.docstatus <> 'VO' group by l.m_attributesetinstance_id)";
*/
/*
			String sql2 = "select count('*') as found from c_order o,c_orderline l "+                      	                      
		                  "  where o.c_order_id = l.c_order_id and o.docstatus <> 'VO' and l.m_attributesetinstance_id = " + M_AttributeSetInstance_ID ;
*/
     		String sql2 = "select count(*) as found	from c_order o,c_orderline l,pp_product_bomline bl "+
                          "where o.c_order_id = l.c_order_id and o.docstatus <> 'VO' "+ 
	                      "  and l.m_attributesetinstance_id=bl.m_attributesetinstance_id"+
	                      "  and bl.PP_Product_BOM_ID = "+PP_Product_BOM_ID+ 
	                      "  and l.m_attributesetinstance_id = "+M_AttributeSetInstance_ID;

		                   
//		                  "  group by l.m_attributesetinstance_id";
//			String descpo = DB.getSQLValueString("description", sql2);
//			System.out.println(descpo);
			int f = DB.getSQLValue("found", sql2);
			System.out.println("Array :"+ a + " Found :" + f);
			if (f==0){		
				data.add(line);
			}
		}
