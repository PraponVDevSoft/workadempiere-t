	...
	implements ListSelectionListener
{
	...
	private void jbInit() throws Exception
	{
		...
		// mainPanel.WIDTH = 750;  aon
		// mainPanel.setSize(750,540); // aon
...
// aon		new ColumnInfo(Msg.translate(Env.getCtx(), "SerNo"), "asi.SerNo", String.class),   
// aon		new ColumnInfo(Msg.translate(Env.getCtx(), "GuaranteeDate"), "asi.GuaranteeDate", Timestamp.class),   
		...
		new ColumnInfo(Msg.translate(Env.getCtx(), "(Product)"), "p.Value", KeyNamePair.class, "p.M_Product_ID"), // @Trifon - Not sure if this need to be shown
		new ColumnInfo(Msg.translate(Env.getCtx(), "(Attribute Set)"), "st.Name", KeyNamePair.class, "st.M_AttributeSet_ID"), // @Trifon - Not sure if this need to be shown
		
		//	See RV_Storage
// aon		new ColumnInfo(Msg.translate(Env.getCtx(), "GoodForDays"), "(daysbetween(asi.GuaranteeDate, SYSDATE))-p.GuaranteeDaysMin", Integer.class, true, true, null),
// aon		new ColumnInfo(Msg.translate(Env.getCtx(), "ShelfLifeDays"), "daysbetween(asi.GuaranteeDate, SYSDATE)", Integer.class),
// aon		new ColumnInfo(Msg.translate(Env.getCtx(), "ShelfLifeRemainingPct"), "CASE WHEN p.GuaranteeDays > 0 THEN TRUNC(((daysbetween(asi.GuaranteeDate, SYSDATE))/p.GuaranteeDays)*100) ELSE 0 END", Integer.class),
	};
	/**	From Clause							*/
	...
	//	+ " LEFT OUTER JOIN M_Product pr ON (asi.M_AttributeSet_ID = pr.M_AttributeSet_ID)"  aon
	;
	/** Where Clause						*/ 
	...
	private String	m_sqlNonZero = " AND (s.QtyOnHand<>0)";    //  OR s.QtyReserved<>0 OR s.QtyOrdered<>0   Aon 28/10/2014
	/* private String	m_sqlMinLife = "";  aon 22/10/56 */
...
aon 193/56   */		
		...
		m_sql = m_table.prepareTable (s_layout, s_sqlFrom, s_sqlWhereWithoutWarehouse, false, "asi")
				+ " ORDER BY asi.Lot desc";	//	oldest, smallest first ,  s.QtyOnHand desc 19/3/56
//		+ " ORDER BY asi.GuaranteeDate, s.QtyOnHand";	//	oldest, smallest first, aon
		//
...
*/				
		...
		try
		{
			pstmt = DB.prepareStatement(sql, null);
			pstmt.setInt(1, m_M_Product_ID);
			// aon  pstmt.setInt(2, m_M_Product_ID);
			if ( !showAll.isSelected() ) {
