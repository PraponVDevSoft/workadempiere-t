...
public class VCreateFromInternalUseUI extends VCreateFromInternalUse implements
	...
	public VCreateFromInternalUseUI(GridTab mTab) {
		...
		log.info(tag + "PP_Product_BOM_ID" + PP_Product_BOM_ID);
	...
	private void jbInit() throws Exception {
		...
		String bomDesc = (bom.getDescription()==null)?"":" [ "+bom.getDescription()+" ]";
		txtProduct.setText(bomName + bomDesc);
		lblProduct.setText(Msg.getElement(Env.getCtx(), "M_Product_ID") + " (Model) : ");
...
//									5), 0, 0));
	...
	protected void loadBOMbyBomId(int PP_Product_BOM_ID){
		loadProductBOM(getBOMDataByBomID(PP_Product_BOM_ID));
	}
